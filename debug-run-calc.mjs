...
import { remapBrowserPostsToParserOrder } from "./parser/geo/route-sequence.js";
...
const browserPostsRaw = useBrowserFixture
  ? (joaoFixture?.posts ?? null)
  : debugDumpPosts;
const browserPosts =
...
for (const ref of REFERENCE) {
  ...
  console.log(
    `  ${mark} Post ${String(ref.num).padStart(2)}: err=${err.toFixed(2)}m  page=${p.pageNum}`,
  );
}
...
console.log(
  `\nTop 5 offenders: ${errs
    .slice(0, 5)
    .map((e) => `Post ${e.num} (${e.err.toFixed(1)}m)`)
    .join(", ")}`,
);
