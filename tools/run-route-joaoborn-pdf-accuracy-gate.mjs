#!/usr/bin/env node
/**
 * João Born PDF-pipeline accuracy gate — exit 1 on any backward regression.
 *
 * Run:  node tools/run-route-joaoborn-pdf-accuracy-gate.mjs
 * Refresh baseline after intentional improvement:
 *   JOAOBORN_UPDATE_BASELINE=1 node tools/run-route-joaoborn-pdf-accuracy-gate.mjs
 */
...
const GT_PATH = path.join(FIXTURES, "joaoborn-ground-truth.json");
const BASELINE_PATH = path.join(FIXTURES, "joaoborn-pdf-baseline.json");
...
async function main() {
  console.log("João Born PDF accuracy gate…");
  ...
  const updateBaseline =
    process.env.JOAOBORN_UPDATE_BASELINE === "1" || !existsSync(BASELINE_PATH);
