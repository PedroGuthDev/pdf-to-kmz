...
// ── Debug helpers ──────────────────────────────────────────────────────────
      ...
      function dumpCalibrationData(parseResult) {
        ...
        if (parseResult.pageDimensions instanceof Map) {
          for (const [pn, dim] of parseResult.pageDimensions) {
            lines.push(
              `  Page ${pn}: ${dim.w.toFixed(2)} × ${dim.h.toFixed(2)} pt`,
            );
          }
        } else {
        ...
        if (parseResult.utmGridPathsPerPage instanceof Map) {
          ...
          if (scaleFactor !== null) {
            lines.push(
              `  → Using scale factor: ${scaleFactor.toFixed(6)} m/pt`,
            );
            lines.push(
            ...
            lines.push(
              `  → NO scale factor found — all posts will get lat: null`,
            );
          }
        ...
        if (Array.isArray(parseResult.viewportBoxes)) {
          ...
          if (parseResult.viewportBoxes.length === 0) {
            lines.push(
              "  (none found — Padrão layer rects may not be extractable)",
            );
          }
        } else {
      ...
      const lineDescriptionInput = document.getElementById(
      ...
      function resolveKmzFilename(userInput, pdfFileName) {
        ...
        base =
          base.replace(/[<>:"/\\|?*\x00-\x1f]/g, "_").replace(/^\.+/, "") ||
          "rota";
      ...
      async function handlePdfFile(file) {
        ...
        if (file.size > 50 * 1024 * 1024) {
          showStatus(
            "O arquivo passa de 50 MB. Selecione um PDF menor.",
            "error",
          );
          return;
        }
      ...
      secondAnchorToggle.addEventListener("click", () => {
        ...
        secondAnchorToggle.setAttribute(
          "aria-expanded",
          open ? "true" : "false",
        );
      });
      ...
      calcBtn.addEventListener("click", async () => {
        ...
        const branchStarts = connections.filter(
          (c) => c.from !== c.to - 1,
        ).length;
        ...
        downloadKmzHint.style.display =
          placemarkEligible === 0 ? "block" : "none";
        downloadKmzBtn.textContent = "Baixar KMZ";
        downloadKmzBtn.removeAttribute("aria-busy");
      });
      ...
      fillRef34Btn.addEventListener("click", () => {
        ...
        document.getElementById("gpsInputLast").value =
          `${ref34.lat}, ${ref34.lon}`;
        compareOutput.style.display = "none";
      });
