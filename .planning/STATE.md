...
| 260603-n4k | DIAGNOSIS (docs-only) — root-caused the LC post-symbol-assignment collapse spun out of 260603-jk7. Posts 9,10,11 get pole `x,y` ~200pt from their correct number-label anchors; **10 & 11 collapse onto post 22's symbol (305,302)**. Stage = `assignPolesGloballyByLabels` (post-positioning.js:1554). Same drawing pathology as the labels: page-4 cable runs 7→8→**11**→9→10 (post 11 out of route order) + posts 9,10 OFF main cable (d>30) → `isOffRouteCablePost` drops them to a shared fallback symbol AND the monotonic arc-window assignment mis-resolves. Fix is post-positioning (Siriu-critical), NOT label code. No code change; `260603-n4k-ROOTCAUSE.md`. All 4 gates green at baseline | 2026-06-03 | — | [260603-n4k-debug-lc-post-symbol-assignment-collapse](./quick/260603-n4k-debug-lc-post-symbol-assignment-collapse/) |

---
Last activity: 2026-06-03 - Quick task 260603-n4k: diagnosed the LC post-symbol-assignment collapse (posts 9/10/11 → wrong/shared poles at 305,302) in assignPolesGloballyByLabels; root cause is the out-of-route-order cable + off-cable posts. The real LC lever is post-positioning, not label assignment. Parser pristine; all 4 gates green.
